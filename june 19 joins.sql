...
-- JOINS
-- INNER JOIN
SELECT customer.cid, orders.oid, customer.cname, orders.amt 
FROM orders 
INNER JOIN customer ON orders.cid = customer.cid;

-- LEFT OUTER JOIN
SELECT orders.oid, orders.amt, products.pid, products.pname 
FROM products 
LEFT OUTER JOIN orders ON orders.pid = products.pid;

-- RIGHT OUTER JOIN
SELECT orders.oid, orders.pid, payment.pay_id, payment.amount 
FROM payment 
